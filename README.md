...
- **Name + Comments**: Поля служат для указания наименования и описания тест-кейса соответственно. ВНИМАНИЕ! Поле "Name" также при необходимости может содержать id связанного кейса при использовании Allure TMS. Для указания связи необходимо перед названием теста указать id кейса из TMS в квадратных скобках, после него название кейса (например, "\[1425\] Авторизация с персональным аккаунтом").
...
## Ограничения
1. При записи файлов с данными шага в целях безопасности плагин автоматически подменяет в запросе хедеры "Authorization", "X-Api-Token", "X-Api-Key" и "X-Jwt-Assertion" на значение "XXX (Has been replaced for safety)". Подмена не настраивается и происходит всегда.
